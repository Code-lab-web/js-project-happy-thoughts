...
document.addEventListener(
  ...
  function (event) {
    ...
    for (var i = 0; i < dfnMapTarget; i += 1) links[i] = document.links[i];
    var inc = 100;
...
function dfnShow(event) {
  ...
  if (dfnMapDone == dfnMapTarget) {
    ...
    while (
      node &&
      (node.nodeType != event.target.ELEMENT_NODE || node.tagName != "DFN")
    )
    ...
    if (node) {
      ...
      if (node.id in dfnMap || node.parentNode.id in dfnMap) {
        ...
        for (var i = 0; i < sourceLinks.length; i += 1) {
          ...
          if (!link.id) link.id = "dfnReturnLink-" + dfnUniqueId++;
          a.href = "#" + link.id;
          if (header != lastHeader) {
            ...
            while (cloneHeader.hasChildNodes())
              ...
              else a.appendChild(cloneHeader.firstChild);
            lastLi = li;
            li.appendChild(a);
            ul.appendChild(li);
          } else {
...
function dfnGetCaption(link) {
  ...
  while (
    node &&
    !(
      node.parentNode.tagName == "DIV" && node.parentNode.className == "section"
    )
  )
    node = node.parentNode;
  while (
    node &&
    (node.nodeType != node.ELEMENT_NODE || !node.tagName.match(/^H[1-6]$/))
  )

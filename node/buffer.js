...
function copyImpl(source, target, targetStart, sourceStart, sourceEnd) {
  ...
  if (targetStart === undefined) {
    ...
    targetStart = NumberIsInteger(targetStart)
      ? targetStart
  ...
  if (sourceStart === undefined) {
    ...
    sourceStart = NumberIsInteger(sourceStart)
      ? sourceStart
  ...
  if (sourceEnd === undefined) {
    ...
    sourceEnd = NumberIsInteger(sourceEnd)
      ? sourceEnd
...
Buffer.prototype.toString = function toString(encoding, start, end) {
  ...
  else start = MathTrunc(start) || 0;

  if (end === undefined || end > len) end = len;
  else end = MathTrunc(end) || 0;
...
Buffer.prototype[customInspectSymbol] = function inspect(recurseTimes, ctx) {
  ...
  let str = StringPrototypeTrim(
    RegExpPrototypeSymbolReplace(/(.{2})/g, this.hexSlice(0, actualMax), "$1 ")
  );
  if (remaining > 0)
...
if (internalBinding("config").hasIntl) {
  ...
  transcode = function transcode(source, fromEncoding, toEncoding) {
    ...
    const err = genericNodeError(`Unable to transcode Buffer [${code}]`, {
      code: code,
      errno: result,
    });
    throw err;
...
defineLazyProperties(module.exports, "internal/blob", [
  "Blob",
  "resolveObjectURL",
]);
defineLazyProperties(module.exports, "internal/file", ["File"]);
